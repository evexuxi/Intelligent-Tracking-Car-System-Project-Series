...
# vgg11 的 cfg: [64, 'M', 128, 'M', 256, 256, 'M', 512, 512, 'M', 512, 512]
# 预先设定新的 cfg，根据设定的 cfg 生成剪枝模型
cfg = [64, 'M', 128, 'M', 256, 256, 'M', 256, 256, 'M', 256, 256]
# 存放生成的剪枝掩膜，掩膜中保留通道的对应值为 1，剪枝通道的对应值为 0
cfg_mask = []
layer_id = 0
# 遍历模型
for m in model.modules():
    if isinstance(m, nn.Conv2d):
        ...
        # 若当前卷积层的输出通道数等于 cfg 中设定的输出通道数，则当前卷积层全部保留，剪枝掩膜为全 1
        if out_channels == cfg[layer_id]:
            cfg_mask.append(torch.ones(out_channels))
            layer_id += 1
            continue
        # 计算当前卷积层每一个卷积核的 L1 范数
        weight_copy = m.weight.data.abs().clone()
        weight_copy = weight_copy.cpu().numpy()
        L1_norm = np.sum(weight_copy, axis=(1, 2, 3))
        # 对每一个卷积核的 L1 范数排序，得到排序结果的索引
        arg_max = np.argsort(L1_norm)
        # 根据 cfg 中对应层所需保留的输出通道数 n 和排序结果，从大到小取前 n 个输出通道的索引
